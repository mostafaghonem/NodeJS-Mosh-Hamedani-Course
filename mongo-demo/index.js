...
const courseSchema = new mongoose.Schema({
  ...
  price: {
    type: Number,
    required: function () {
      return this.isPuplished;
    }, //Note :here we can not use arrow fnc as does not have its own this
...
async function getCourses() {
  const course = await Course.find({ autor: 'Mosh', isPuplished: true }) //here find and other fncs take a filter this filter is obj
...
async function updateCourse(id) {
  ...
  const coures = await Course.findByIdAndUpdate(
    ...
    { new: true }
  ); //the third argument is for return the updated document
  console.log(coures);
}
